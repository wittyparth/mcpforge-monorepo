...
def _redis() -> Redis:
    """Return a Redis client bound to the shared pool. Caller closes it."""
    raise NotImplementedError("Use _with_redis() instead")


async def _with_redis():  # type: ignore[no-untyped-def]
    """Context manager that yields a Redis client from the shared pool."""
...
async def get_status(identifier: str | UUID) -> LockoutStatus:
    ...
    async for r in _with_redis():
        ...
        return LockoutStatus(
            locked=False, attempts=attempts, retry_after=0
        )
    return LockoutStatus(locked=False, attempts=0, retry_after=0)  # type: ignore[unreachable]
...
async def record_failure(identifier: str | UUID) -> LockoutStatus:
    ...
    async for r in _with_redis():
        ...
        # Set TTL on the first failure (and refresh on each so the window
        # extends only on continuous failures).
